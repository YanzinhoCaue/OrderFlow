  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-white/80 dark:bg-white/5 border-2 border-amber-500/20 rounded-2xl px-6 py-4 shadow-xl backdrop-blur-xl">
        <div>
          <p className="text-sm text-stone-600 dark:text-stone-400">Gestão de menu</p>
          ...
          <p className="text-xs text-stone-500 dark:text-stone-500 mt-1">
            Organize categorias, pratos e disponibilidade
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <IngredientsManager restaurantId={(restaurant as any).id} buttonText="Ingredientes" />
          <MenuActions restaurantId={(restaurant as any).id} buttonText="Categoria" />
          <NewDishModal 
